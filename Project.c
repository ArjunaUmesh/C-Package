...
{   //To get the details from user :
    ...
    for(int i=0;i<9;i++)
    {   for(int j=0;j<9;j++)
        {   if(puzzGrid[i][j]<0 || puzzGrid[i][j]>9)
...
//Function to print entire puzzle-grid in terminal and file - to create a visual representation
 void print()
{     printf("\n\n");
      file=fopen("d://sudoku.txt","a");
...
void getDetails()
{
    ...
    printf("In classic sudoku, the objective is to fill a 9*9 grid with digits so that each column, each row, and each of the nine 3*3 subgrids that compose the grid contain all of the digits from 1 to 9.\n\n");
    printf("Input data (1) manually or from (2)file : \n1.Manual\n2.From File - problem.txt\n");
    scanf("%d",&choice);
    system("cls");
    if(choice==1){
        //To get values from user manually line by line
